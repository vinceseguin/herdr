    ...
    Command::new("agent")
        ...
        .subcommand(
            Command::new("start")
                ...
                .after_help(
                    "The pane must be at its interactive shell prompt. Success means the expected agent was detected in the same terminal and is ready for input.\n\n--account applies one of the profiles from `herdr account list` by exporting CLAUDE_CONFIG_DIR in the pane's shell before the agent is started; without it the default profile is used when one is configured. The response adds account and account_state, and account_state is only ok when the launched process's own environment was read and names that profile.\n\nnext: herdr agent prompt <TARGET> <TEXT> --wait",
                ),
        )
        .subcommand(
            Command::new("explain")
