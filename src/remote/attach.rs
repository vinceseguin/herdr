...
#[derive(Debug, Clone)]
// `pub(crate)`: consumed by `src/fleet/transport/ssh.rs` (E1 PR 3).
...
impl RemoteSsh {
    pub(crate) fn new(target: String, manage_ssh_config: bool, session_name: String) -> Self {
...
///
/// Unlike [`find_installed_remote_herdr`] it does not require surface-interest
/// support (a generation-1 server of any version qualifies), never reads
/// `HERDR_REMOTE_BINARY`, never installs or uploads a binary, never stops or
/// hands off a remote server, and never prompts. A host with no compatible
/// herdr is `Ok(None)`; only transport failures are `Err`.
// Consumed by `src/fleet/transport/ssh.rs` (E1 PR 3); `herdr --remote` keeps
// using `prepare_remote_herdr`.
pub(crate) fn discover_remote_herdr(ssh: &RemoteSsh) -> io::Result<Option<RemoteHerdr>> {
    let platform = detect_remote_platform(ssh)?;
    let remote_herdr = RemoteHerdr::for_platform(platform);
    let candidates = remote_binary_candidates(ssh, &remote_herdr)?;
    // Candidate probe failures are "not compatible" (a candidate can disappear
    // between discovery and the probe); a failure probing the default install
    // path propagates, matching `prepare_remote_herdr`.
    for candidate in &candidates {
        if remote_binary_supports_endpoint_requirement(ssh, candidate, false).unwrap_or(false) {
            return Ok(Some(candidate.clone()));
        }
    }
    if remote_binary_supports_endpoint_requirement(ssh, &remote_herdr, false)? {
        return Ok(Some(remote_herdr));
    }
    Ok(None)
}
...
// `pub(crate)`: pinned by `src/fleet/transport/ssh.rs` tests (E1 PR 3).
...
impl SshStdioBridge {
    ...
    /// `errors` instead of the process stderr or the log.
    // Consumed by `src/fleet/transport/ssh.rs` (E1 PR 3).
                ...
                match listener.accept() {
                    Ok(stream) => {
                        ...
                        if let Err(err) = bridge_connection(
                            ...
                            errors.report(BridgeFailure::Connection, &err);
                        }
                    }
                    ...
                    Err(err) => {
                        errors.report(BridgeFailure::Listener, &err);
                        break;
                    }
                }
            }
...
/// Which part of [`SshStdioBridge`]'s accept loop failed.
#[derive(Clone, Copy)]
enum BridgeFailure {
    /// One bridged connection's ssh child failed; the listener keeps going.
    Connection,
    /// The listener itself failed; the accept loop ends.
    Listener,
}

/// Where [`SshStdioBridge`]'s accept thread reports per-connection failures.
///
/// `Stderr` keeps `herdr --remote`'s output byte-for-byte unchanged and `Log`
/// keeps the saved-machine (noninteractive) bridge's tracing output unchanged.
/// `Report` lets a long-lived caller turn a bridge failure into host-local
/// state instead of terminal noise. The callback runs on the bridge's accept
/// thread between connections, so it must return promptly; a panic inside it
/// is contained and logged rather than allowed to kill the listener.
#[derive(Clone)]
...
impl BridgeErrorSink {
    fn report(&self, failure: BridgeFailure, err: &io::Error) {
        match (self, failure) {
            (Self::Stderr, BridgeFailure::Connection) => {
                eprintln!("herdr: remote bridge failed: {err}");
            }
            (Self::Stderr, BridgeFailure::Listener) => {
                eprintln!("herdr: remote bridge listener failed: {err}");
            }
            (Self::Log, BridgeFailure::Connection) => {
                tracing::warn!(error = %err, "saved SSH endpoint bridge failed");
            }
            (Self::Log, BridgeFailure::Listener) => {
                tracing::warn!(error = %err, "saved SSH endpoint listener failed");
            }
            (Self::Report(report), failure) => {
                let message = match failure {
                    BridgeFailure::Connection => format!("remote bridge failed: {err}"),
                    BridgeFailure::Listener => format!("remote bridge listener failed: {err}"),
                };
                let outcome =
...
"#
    ...
    fn prepare_remote_herdr_returns_first_generation_one_candidate() {
        ...
        let prepared = without_remote_binary_override(|| {
            let ssh = RemoteSsh::new(
                "charact-host".to_string(),
                false,
                crate::session::DEFAULT_SESSION_NAME.to_string(),
            );
            prepare_remote_herdr(&ssh, false, false)
        })
    ...
    fn bridge_error_sink_reports_a_failing_ssh_child() {
        ...
        // Keep the client open until the failure is reported: since upstream
        // #3670 a connection whose *client* hung up first is not a failure,
        // whatever the ssh child's exit status — exactly the case the fleet
        // cares about is the child dying under a live client.
    ...
    fn bridge_survives_a_panicking_error_sink() {
        ...
        for _ in 0..2 {
            ...
            // The client stays open until the report lands (see the test above).
            let client = crate::ipc::connect_local_stream(&socket).expect("connect to bridge");
            let deadline = Instant::now() + Duration::from_secs(10);
